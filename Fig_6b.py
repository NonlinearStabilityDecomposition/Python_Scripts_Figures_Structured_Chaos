...
# Daten einlesen
data = pd.read_csv("Daten5.txt", sep='\s+', header=None, names=["Z", "KDF"])
...
# Berechnung einer monoton fallenden Version der LOESS-Kurve
loess_monotonic = loess_smoothed.copy()
min_val = loess_monotonic[0, 1]
for i in range(1, len(loess_monotonic)):
    if loess_monotonic[i, 1] > min_val:
        loess_monotonic[i, 1] = min_val
    else:
        min_val = loess_monotonic[i, 1]
...
ax2 = ax1.twinx()
ax2.hist(data["Z"], bins=30, color='lightgray', alpha=0.3, edgecolor='black', label='Z Histogram')
...
z_range = np.linspace(50, 10000, 100)
...
#ax1.plot(loess_monotonic[:, 0], loess_monotonic[:, 1], linestyle='--', color='red', linewidth=2, label='LOESS Monotonic Lower Bound')
